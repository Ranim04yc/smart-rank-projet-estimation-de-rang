...
TOKEN_FILE_TOKEN = read_token_file()
...
def main() -> None:
    """Run the bot."""
    # Get bot token from (in order of priority):
    # 1. Environment variable
    # 2. config.py file
    # 3. token.txt file
    # 4. Placeholder
    bot_token = os.getenv("TELEGRAM_BOT_TOKEN") or CONFIG_TOKEN or TOKEN_FILE_TOKEN or "#YOUR_TOKEN"
    
    if bot_token == "#YOUR_TOKEN" or not bot_token or bot_token.strip() == "":
        logger.error(
            "❌ TELEGRAM_BOT_TOKEN not configured!\n\n"
            "Please configure it using ONE of these methods:\n"
            "1. Edit token.txt and replace #YOUR_TOKEN with your token (EASIEST)\n"
            "2. Edit config.py and set TELEGRAM_BOT_TOKEN = 'your_token_here'\n"
            "3. Set environment variable: $env:TELEGRAM_BOT_TOKEN='your_token_here'\n\n"
            "To get a token:\n"
